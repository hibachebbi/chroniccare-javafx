...
public class EventService {
    ...
    /** Table créée avant migration QR : pas de qr_code_token / qr_code_path. */
    private Boolean inscriptionQrColumnsCached;
    ...
    public boolean registerPatientToEvent(int eventId, User patient) throws SQLException {
        ...
        try {
                ...
                try {
                    EmailService.sendRegistrationEmailWithQRCode(
                            ...
                            event.getDateDebut() != null
                                    ? event.getDateDebut().format(java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"))
                                    : "",
                            event.getLieu(),
                            qrPath
                    );
            ...
            if (hasInscriptionQrColumns() && qrToken != null) {
                try {
    ...
    private EventRegistration mapRegistration(ResultSet rs) throws SQLException {
        ...
        if (resultSetHasColumn(rs, "qr_code_token")) {
    ...
    private boolean hasInscriptionQrColumns() throws SQLException {
        if (inscriptionQrColumnsCached != null) {
            return inscriptionQrColumnsCached;
        }
        DatabaseMetaData meta = conn.getMetaData();
        boolean token = false;
        try (ResultSet cols = meta.getColumns(null, null, "inscription_evenement", "qr_code_token")) {
            token = cols.next();
        }
        if (!token) {
            try (ResultSet cols = meta.getColumns(null, null, "INSCRIPTION_EVENEMENT", "qr_code_token")) {
        ...
        inscriptionQrColumnsCached = token;
        return token;
    }

    private static boolean resultSetHasColumn(ResultSet rs, String columnLabel) throws SQLException {
