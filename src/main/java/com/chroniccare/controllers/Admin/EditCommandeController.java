...
public class EditCommandeController {
    ...
    public void setCommande(Commande commande) {
        ...
        if (commande == null) {
            showAlert(Alert.AlertType.WARNING, "ChronicCare", null,
                    "Aucune commande sélectionnée. Retournez à la liste et choisissez une commande à modifier.");
            setFormDisabled(true);
            return;
        }
    ...
    public void handleSave() {
        ...
        try {
            ...
            currentCommande.setCreatedAt(
                    currentCommande.getCreatedAt() == null ? LocalDateTime.now() : currentCommande.getCreatedAt());
    ...
    private void setFormDisabled(boolean disabled) {
        if (statutCombo != null)
            statutCombo.setDisable(disabled);
        if (paiementCombo != null)
            paiementCombo.setDisable(disabled);
        if (saveButton != null)
    ...
    private boolean isTransitionAllowed(String from, String to) {
        ...
        if (equalsIgnoreCase(f, t))
            return true;

        // Statuts terminaux
        if (equalsIgnoreCase(f, "livree"))
            return false;
        if (equalsIgnoreCase(f, "annulee"))
            return false;
